...
def score_stock(info, ind):
    ...
    if stoch < 20:
        tech_score += 14
        signals.append("Stoch Aşırı Sat")
    elif stoch > 82:
        tech_score -= 8
    ...
    if ema_golden:
        tech_score += 14
        signals.append("Altın Kesişim")
    if wr < -80:
        tech_score += 10
        signals.append("Williams Aşırı Sat")
    if abs(cci) > 150:
        tech_score += 6
    if mom > 5:
        tech_score += 8
    ...
    if 0 < pe < 8:
        fund_score += 25
        signals.append("Çok Ucuz F/K")
    elif 0 < pe < 14:
        fund_score += 14
        signals.append("Ucuz F/K")
    elif pe > 25:
        fund_score -= 8

    if 0 < pb < 0.8:
        fund_score += 22
        signals.append("Defter Altı")
    elif 0 < pb < 1.5:
        fund_score += 10

    if rev_growth > 30:
        fund_score += 20
        signals.append("Güçlü Büyüme")
    elif rev_growth > 10:
        fund_score += 10
    elif rev_growth < -5:
        fund_score -= 8
    ...
    if vol_mult > 4:
        ...
        flags.append({"icon": "🔴", "detail": f"{vol_mult:.1f}x hacim"})
    elif vol_mult > 2.5:
        vol_score += 25
...
def run_full_scan():
    ...
    print(f"Tarama tamamlandı — {len(results)} hisse — {elapsed}s")
...
def main():
    ...
    if st.session_state.has_data and len(st.session_state.results) > 0:
        ...
        if len(ticker_options) > 0:
            selected_ticker = st.selectbox("Hisse Seç", ticker_options)
